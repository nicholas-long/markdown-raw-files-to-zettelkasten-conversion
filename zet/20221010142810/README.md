...
- learned how to exploit when developers do it the wrong way
...
- mounting development directory is an antipattern because you might forget to turn it off before releasing
  - why run things differently while testing something that's supposed to work the same everyhwere?
- `ADD . .` is an antipattern
  - the Dockerfile is in the directory, so it starts tracking its own changes and does not cache any build steps after the `ADD` line
    - ".dockerignore" ?
- need privileged mode for lower level tools
  - not really, use capabilities for finer-grained access control
  - run attack scenarios as scripted test cases - why not?
